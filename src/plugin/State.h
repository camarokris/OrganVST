...
inline bool readProjectState(Steinberg::IBStream* stream,ProjectState& result) {
  ...
  if(!s.readInt32u(magic)||magic!=0x4F565354||!s.readInt32u(version)||(version!=1&&version!=2)||!text(result.path,1048576)||!s.readDouble(result.gain)||!std::isfinite(result.gain)||result.gain<0||result.gain>1)return false;
  if(version==1) {
...
inline bool writeProjectState(Steinberg::IBStream* stream,const ProjectState& state) {
  ...
  if(!s.writeInt32u(0x4F565354)||!s.writeInt32u(2)||!text(state.path)||!s.writeDouble(state.gain)||!s.writeInt32u(Steinberg::uint32(state.controls.size())))return false;
