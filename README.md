...
Homebrew packages `wxwidgets wavpack fftw yaml-cpp pkg-config`.
...
Insert OrganVST as an instrument. Choose **Load organ** and select an extracted
GrandOrgue-compatible `.organ` or `.odf` file. WAV and WavPack samples are loaded
...
learn, no auxiliary routing, no full tabbed interface/console, no ZIP-pack picker,
